...
## 7.0 ข้อสรุปของทั้งคอร์ส

ถ้าจะจำอะไรไปแค่อย่างเดียวจากคอร์สนี้ ให้จำข้อนี้:

> **ด่านที่มีค่าคือด่านตอน merge ไม่ใช่ด่านตอน deploy**

เพราะโค้ดเสียจะขึ้น production ได้ก็ต้องเข้า main ก่อน ถ้าปิดทางเข้า main ไว้
ก็ไม่ต้องไปกังวลว่าใครเป็นคนสั่ง deploy

นั่นแปลว่า **branch protection สำคัญกว่าการเลือกระหว่างทาง A กับทาง B**
ทำ branch protection แล้วใช้ทาง A ปลอดภัยกว่าใช้ทาง B แต่ไม่ทำ branch protection
เยอะมาก — เพราะแบบหลังยังปล่อยให้ push ตรงเข้า main ได้
...
**คำตอบสำหรับโปรเจกต์นี้และเว็บส่วนใหญ่: ทาง A + branch protection**

ได้ Preview URL ฟรี ไม่มี secret ให้ดูแล ไม่มี workflow ให้ maintain และโค้ด
ที่เทสไม่ผ่านก็เข้า main ไม่ได้อยู่ดี นี่ไม่ใช่ "ทางลัด" — มันคือทางที่ถูกจริง
เมื่อไม่มีเหตุผลให้ทำมากกว่านั้น

**ย้ายไปทาง B เมื่อมีเหตุผลเฉพาะเจาะจง** อย่างน้อยหนึ่งข้อ:

- ต้อง deploy artifact ตัวเดียวกับที่เทส (`--prebuilt`) ไม่ใช่ของที่ Vercel build ใหม่
- ต้องมีคนกดอนุมัติก่อนขึ้น production
- ต้องรัน migration / smoke test / warm cache คั่นก่อนหรือหลัง deploy
- ต้องมี audit trail ว่าใครสั่ง deploy อันไหน ในที่เดียวกับ CI
- เป็น monorepo ที่ต้องเลือก deploy เฉพาะบางส่วน

**อย่าย้ายไปทาง B เพราะรู้สึกว่ามันดูมืออาชีพกว่า** ทุก secret คือของที่ต้องหมุน
ทุก workflow คือของที่ต้องซ่อม ความซับซ้อนที่ไม่ได้แลกมาด้วยประโยชน์จริงคือหนี้
